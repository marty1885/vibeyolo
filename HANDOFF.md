...
1. **Attention block (PSA / A2C2f)** *(medium)* — composes already-validated convs (L34-L38 model.10, L88-L92 model.22) with `softmax16` + fp16_fma attention scores. New work is the QKV split + attention-matmul + residuals around the proj and FFN convs (proj/FFN convs themselves already pass standalone).
