...
- [x] **Attention block — `/model.22` A2C2f** (`integ/attn_model22/`) — **parametric clone of `/model.10`.** ONNX confirms `/model.22/m.0/m.0.1/...` (the A2C2f inner attention sub-block) is byte-identical in shape to `/model.10/m/m.0/...` (HEADS=2 N=400 DIM_Q=32 DIM_V=64 C_QKV=256 C_FE=128 20²). The residual source for the post-proj Add is `/model.22/m.0/m.0.0/Add_output_0` (output of the m.0.0 conv-pair upstream of the attention sub-block in A2C2f). Same `attn` structural module copied verbatim; per-block `attn_scales_pkg` regenerated by the new extract.py — mirrors how `upsample_model14` copy-clones `upsample_model11`. 10/10 checks, 5/5 ORT samples cos 0.99968..0.99981, **74,911 cyc/frame under T_FRAME=100k**.
