...
1. **`hw/ip/flash_attn/` — systolic flash-attention leaf IP** *(medium)* — parameterized (HEADS, N, DIM_Q, DIM_V, BR, BC) systolic fp16 MAC mesh with online (tiled) softmax. Never materializes the full N×N scores tensor — running `(m_i, l_i)` per row updated tile-by-tile. Reusable across PSA (`/model.10`) and A2C2f (`/model.22`); both have byte-identical shapes (heads=2, N=400, DIM_Q=32, DIM_V=64). Standalone DV: `flash_attn_ref.sv` golden running plain dense softmax-attention + random Q/K/V sweep + edge cases. Budget at P≈512 cells: ~60k cyc/pass, fits T_FRAME=100k. **Open question before locking IP boundary:** does `pe` (depthwise on V) live inside the IP (folded into the V-tile load) or stay outside (post-reduction add)? Verify against ONNX — flash-attention reduces tile-by-tile, so pe-add post-reduction stays outside.
2. **Attention integration shim — `/model.10` PSA** *(small)* — replaces behavioral `integ/attn_model10/` with ~80-line shim instantiating `flash_attn` + `add_rq` residuals + requant boundaries. Re-run existing extract.py / TB.
