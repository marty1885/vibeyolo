...
#
# golden.py — Stimulus + reference for tiled layer_3 (YOLO26n
# /model.2/m.0/cv1 Conv-BN-SiLU, 3x3, 16 -> 8, stride 1, pad 1).
# Input is /model.2/Slice_1_output_0 (the 16-ch second half of /model.2/cv1).
#
# Mirrors integ/layer_1/golden.py.
...
W_q  = init_by_name['onnx::Conv_1660_quantized'].astype(np.int32)
s_w  = float(init_by_name['onnx::Conv_1660_scale'])
zp_w = int(init_by_name['onnx::Conv_1660_zero_point'])
bias = init_by_name['onnx::Conv_1661'].astype(np.float32)
assert zp_w == 0 and W_q.shape == (NCH_OUT, NCH_IN, K, K)
sum_w = W_q.reshape(NCH_OUT, -1).sum(axis=1).astype(np.int32)

TARGET_OUT = '/model.2/m.0/cv1/act/Mul_output_0'
IN_Q  = '/model.2/Slice_1_output_0_quantized'
IN_S  = '/model.2/Slice_1_output_0_scale'
IN_ZP = '/model.2/Slice_1_output_0_zero_point'
...
def hw_reference(qx_full, s_a, zp_a):
    ...
    H_F = qx_full.shape[2]; W_F = qx_full.shape[3]
    sr0, sr1 = max(0,r0), min(H_F,r1); sc0, sc1 = max(0,c0), min(W_F,c1)
...
for sname, kind, seed in samples:
    ...
    ort_full, qx_full, s_a, zp_a = run_ort(img)
    if s_a == 0.0: s_a = 1.0
    ort_roi = ort_full[0, :, R:R+OUT_H, C:C+OUT_W]
    hw_out, bias_eff, x_i8, s_acc = hw_reference(qx_full, s_a, zp_a)
