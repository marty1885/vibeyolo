...
//
// Generated skeleton for L80: /model.23/one2one_cv3.1/one2one_cv3.1.0/one2one_cv3.1.0.1/conv/Conv_quant  (SILU=0)

module layer_80
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_80_COUT,
  parameter int  CIN        = LAYER_80_CIN,
  parameter int  K          = LAYER_80_K,
  parameter int  STRIDE     = LAYER_80_STRIDE,
  parameter int  PAD        = LAYER_80_PAD,
  parameter int  P_COUT     = LAYER_80_P_COUT,
  parameter int  P_CIN      = LAYER_80_P_CIN,
  parameter int  P_PIX      = LAYER_80_P_PIX,
  ...
  for (genvar pix = 0; pix < P_PIX; pix++) begin : gen_pix
    ...
    conv_layer #(
      ...
      .H_OUT(LAYER_80_H), .W_OUT(LAYER_80_W),
