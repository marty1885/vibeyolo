...
//
// Generated skeleton for L95: /model.23/one2one_cv3.2/one2one_cv3.2.0/one2one_cv3.2.0.0/conv/Conv_quant  (SILU=1)

module layer_95
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_95_COUT,
  parameter int  CIN        = LAYER_95_CIN,
  parameter int  K          = LAYER_95_K,
  parameter int  STRIDE     = LAYER_95_STRIDE,
  parameter int  PAD        = LAYER_95_PAD,
  parameter int  P_COUT     = LAYER_95_P_COUT,
  parameter int  P_CIN      = LAYER_95_P_CIN,
  parameter int  P_PIX      = LAYER_95_P_PIX,
  ...
  for (genvar pix = 0; pix < P_PIX; pix++) begin : gen_pix
    ...
    conv_layer #(
      ...
      .H_OUT(LAYER_95_H), .W_OUT(LAYER_95_W),
