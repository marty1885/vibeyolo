...
//
// Generated skeleton for L65: /model.23/one2one_cv3.0/one2one_cv3.0.1/one2one_cv3.0.1.0/conv/Conv_quant  (SILU=1)

module layer_65
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_65_COUT,
  parameter int  CIN        = LAYER_65_CIN,
  parameter int  K          = LAYER_65_K,
  parameter int  STRIDE     = LAYER_65_STRIDE,
  parameter int  PAD        = LAYER_65_PAD,
  parameter int  P_COUT     = LAYER_65_P_COUT,
  parameter int  P_CIN      = LAYER_65_P_CIN,
  parameter int  P_PIX      = LAYER_65_P_PIX,
  ...
  for (genvar pix = 0; pix < P_PIX; pix++) begin : gen_pix
    ...
    conv_layer #(
      ...
      .H_OUT(LAYER_65_H), .W_OUT(LAYER_65_W),
