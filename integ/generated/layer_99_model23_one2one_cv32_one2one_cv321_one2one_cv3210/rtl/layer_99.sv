...
//
// Generated skeleton for L99: /model.23/one2one_cv3.2/one2one_cv3.2.1/one2one_cv3.2.1.0/conv/Conv_quant  (SILU=1)

module layer_99
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_99_COUT,
  parameter int  CIN        = LAYER_99_CIN,
  parameter int  K          = LAYER_99_K,
  parameter int  STRIDE     = LAYER_99_STRIDE,
  parameter int  PAD        = LAYER_99_PAD,
  parameter int  P_COUT     = LAYER_99_P_COUT,
  parameter int  P_CIN      = LAYER_99_P_CIN,
  parameter int  P_PIX      = LAYER_99_P_PIX,
  ...
  for (genvar pix = 0; pix < P_PIX; pix++) begin : gen_pix
    ...
    conv_layer #(
      ...
      .H_OUT(LAYER_99_H), .W_OUT(LAYER_99_W),
