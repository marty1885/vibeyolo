...
//
// Generated skeleton for L97: /model.23/one2one_cv3.2/one2one_cv3.2.0/one2one_cv3.2.0.1/conv/Conv_quant  (SILU=1)

module layer_97
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_97_COUT,
  parameter int  CIN        = LAYER_97_CIN,
  parameter int  K          = LAYER_97_K,
  parameter int  STRIDE     = LAYER_97_STRIDE,
  parameter int  PAD        = LAYER_97_PAD,
  parameter int  P_COUT     = LAYER_97_P_COUT,
  parameter int  P_CIN      = LAYER_97_P_CIN,
  parameter int  P_PIX      = LAYER_97_P_PIX,
  ...
  for (genvar pix = 0; pix < P_PIX; pix++) begin : gen_pix
    ...
    conv_layer #(
      ...
      .H_OUT(LAYER_97_H), .W_OUT(LAYER_97_W),
