...
//
// Generated skeleton for L100: /model.23/one2one_cv3.2/one2one_cv3.2.1/one2one_cv3.2.1.1/conv/Conv_quant  (SILU=1)

module layer_100
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_100_COUT,
  parameter int  CIN        = LAYER_100_CIN,
  parameter int  K          = LAYER_100_K,
  parameter int  STRIDE     = LAYER_100_STRIDE,
  parameter int  PAD        = LAYER_100_PAD,
  parameter int  P_COUT     = LAYER_100_P_COUT,
  parameter int  P_CIN      = LAYER_100_P_CIN,
  parameter int  P_PIX      = LAYER_100_P_PIX,
  ...
  for (genvar pix = 0; pix < P_PIX; pix++) begin : gen_pix
    ...
    conv_layer #(
      ...
      .H_OUT(LAYER_100_H), .W_OUT(LAYER_100_W),
