...
//
// Generated skeleton for L96: /model.23/one2one_cv2.2/one2one_cv2.2.1/conv/Conv_quant  (SILU=0)

module layer_96
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_96_COUT,
  parameter int  CIN        = LAYER_96_CIN,
  parameter int  K          = LAYER_96_K,
  parameter int  STRIDE     = LAYER_96_STRIDE,
  parameter int  PAD        = LAYER_96_PAD,
  parameter int  P_COUT     = LAYER_96_P_COUT,
  parameter int  P_CIN      = LAYER_96_P_CIN,
  parameter int  P_PIX      = LAYER_96_P_PIX,
  ...
  for (genvar pix = 0; pix < P_PIX; pix++) begin : gen_pix
    ...
    conv_layer #(
      ...
      .H_OUT(LAYER_96_H), .W_OUT(LAYER_96_W),
