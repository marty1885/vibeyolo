...
//
// Generated skeleton for L60: /model.23/one2one_cv3.0/one2one_cv3.0.0/one2one_cv3.0.0.0/conv/Conv_quant  (SILU=1)

module layer_60
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_60_COUT,
  parameter int  CIN        = LAYER_60_CIN,
  parameter int  K          = LAYER_60_K,
  parameter int  STRIDE     = LAYER_60_STRIDE,
  parameter int  PAD        = LAYER_60_PAD,
  parameter int  P_COUT     = LAYER_60_P_COUT,
  parameter int  P_CIN      = LAYER_60_P_CIN,
  parameter int  P_PIX      = LAYER_60_P_PIX,
  ...
  for (genvar pix = 0; pix < P_PIX; pix++) begin : gen_pix
    ...
    conv_layer #(
      ...
      .H_OUT(LAYER_60_H), .W_OUT(LAYER_60_W),
