...
//
// Generated skeleton for L67: /model.19/m.0/cv2/conv/Conv_quant  (SILU=0)

module layer_67
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_67_COUT,
  parameter int  CIN        = LAYER_67_CIN,
  parameter int  K          = LAYER_67_K,
  parameter int  STRIDE     = LAYER_67_STRIDE,
  parameter int  PAD        = LAYER_67_PAD,
  parameter int  P_COUT     = LAYER_67_P_COUT,
  parameter int  P_CIN      = LAYER_67_P_CIN,
  parameter int  P_PIX      = LAYER_67_P_PIX,
  ...
  for (genvar pix = 0; pix < P_PIX; pix++) begin : gen_pix
    ...
    conv_layer #(
      ...
      .H_OUT(LAYER_67_H), .W_OUT(LAYER_67_W),
