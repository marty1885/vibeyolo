...
//
// Generated skeleton for L78: /model.23/one2one_cv3.1/one2one_cv3.1.0/one2one_cv3.1.0.0/conv/Conv_quant  (SILU=1)

module layer_78
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_78_COUT,
  parameter int  CIN        = LAYER_78_CIN,
  parameter int  K          = LAYER_78_K,
  parameter int  STRIDE     = LAYER_78_STRIDE,
  parameter int  PAD        = LAYER_78_PAD,
  parameter int  P_COUT     = LAYER_78_P_COUT,
  parameter int  P_CIN      = LAYER_78_P_CIN,
  parameter int  P_PIX      = LAYER_78_P_PIX,
  ...
  for (genvar pix = 0; pix < P_PIX; pix++) begin : gen_pix
    ...
    conv_layer #(
      ...
      .H_OUT(LAYER_78_H), .W_OUT(LAYER_78_W),
