...
//
// Generated skeleton for L62: /model.23/one2one_cv3.0/one2one_cv3.0.0/one2one_cv3.0.0.1/conv/Conv_quant  (SILU=0)

module layer_62
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_62_COUT,
  parameter int  CIN        = LAYER_62_CIN,
  parameter int  K          = LAYER_62_K,
  parameter int  STRIDE     = LAYER_62_STRIDE,
  parameter int  PAD        = LAYER_62_PAD,
  parameter int  P_COUT     = LAYER_62_P_COUT,
  parameter int  P_CIN      = LAYER_62_P_CIN,
  parameter int  P_PIX      = LAYER_62_P_PIX,
  ...
  for (genvar pix = 0; pix < P_PIX; pix++) begin : gen_pix
    ...
    conv_layer #(
      ...
      .H_OUT(LAYER_62_H), .W_OUT(LAYER_62_W),
