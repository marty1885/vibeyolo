...
//
// Generated skeleton for L83: /model.23/one2one_cv3.1/one2one_cv3.1.1/one2one_cv3.1.1.0/conv/Conv_quant  (SILU=1)

module layer_83
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_83_COUT,
  parameter int  CIN        = LAYER_83_CIN,
  parameter int  K          = LAYER_83_K,
  parameter int  STRIDE     = LAYER_83_STRIDE,
  parameter int  PAD        = LAYER_83_PAD,
  parameter int  P_COUT     = LAYER_83_P_COUT,
  parameter int  P_CIN      = LAYER_83_P_CIN,
  parameter int  P_PIX      = LAYER_83_P_PIX,
  ...
  for (genvar pix = 0; pix < P_PIX; pix++) begin : gen_pix
    ...
    conv_layer #(
      ...
      .H_OUT(LAYER_83_H), .W_OUT(LAYER_83_W),
