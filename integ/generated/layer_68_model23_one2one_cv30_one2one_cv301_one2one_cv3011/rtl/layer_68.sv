...
//
// Generated skeleton for L68: /model.23/one2one_cv3.0/one2one_cv3.0.1/one2one_cv3.0.1.1/conv/Conv_quant  (SILU=1)

module layer_68
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_68_COUT,
  parameter int  CIN        = LAYER_68_CIN,
  parameter int  K          = LAYER_68_K,
  parameter int  STRIDE     = LAYER_68_STRIDE,
  parameter int  PAD        = LAYER_68_PAD,
  parameter int  P_COUT     = LAYER_68_P_COUT,
  parameter int  P_CIN      = LAYER_68_P_CIN,
  parameter int  P_PIX      = LAYER_68_P_PIX,
  ...
  for (genvar pix = 0; pix < P_PIX; pix++) begin : gen_pix
    ...
    conv_layer #(
      ...
      .H_OUT(LAYER_68_H), .W_OUT(LAYER_68_W),
