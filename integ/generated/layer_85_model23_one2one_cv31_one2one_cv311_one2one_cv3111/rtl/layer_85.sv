...
//
// Generated skeleton for L85: /model.23/one2one_cv3.1/one2one_cv3.1.1/one2one_cv3.1.1.1/conv/Conv_quant  (SILU=1)

module layer_85
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_85_COUT,
  parameter int  CIN        = LAYER_85_CIN,
  parameter int  K          = LAYER_85_K,
  parameter int  STRIDE     = LAYER_85_STRIDE,
  parameter int  PAD        = LAYER_85_PAD,
  parameter int  P_COUT     = LAYER_85_P_COUT,
  parameter int  P_CIN      = LAYER_85_P_CIN,
  parameter int  P_PIX      = LAYER_85_P_PIX,
  ...
  for (genvar pix = 0; pix < P_PIX; pix++) begin : gen_pix
    ...
    conv_layer #(
      ...
      .H_OUT(LAYER_85_H), .W_OUT(LAYER_85_W),
