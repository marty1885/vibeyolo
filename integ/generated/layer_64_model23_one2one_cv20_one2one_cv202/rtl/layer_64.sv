...
//
// Generated skeleton for L64: /model.23/one2one_cv2.0/one2one_cv2.0.2/Conv_quant  (SILU=0)

module layer_64
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_64_COUT,
  parameter int  CIN        = LAYER_64_CIN,
  parameter int  K          = LAYER_64_K,
  parameter int  STRIDE     = LAYER_64_STRIDE,
  parameter int  PAD        = LAYER_64_PAD,
  parameter int  P_COUT     = LAYER_64_P_COUT,
  parameter int  P_CIN      = LAYER_64_P_CIN,
  parameter int  P_PIX      = LAYER_64_P_PIX,
  ...
  for (genvar pix = 0; pix < P_PIX; pix++) begin : gen_pix
    ...
    conv_layer #(
      ...
      .H_OUT(LAYER_64_H), .W_OUT(LAYER_64_W),
