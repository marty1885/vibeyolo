...
//
// stem_l0_test — Integration test for tiled YOLO26n /model.0 stem layer.
...
static constexpr double S_OUT_SILU = 64.0 / 127.0;

static constexpr int DOT_LAT     = 1 + 5;  // 1 + clog2(27) = 6
...
int main(int argc, char** argv) {
    ...
    const std::vector<std::string> sample_names = {
        // "all0" excluded — degenerate all-zero ORT output is numerically
        // unstable for cosine and not a meaningful conv DUT test.
        "rand0", "rand1", "half", "all1", "gradient", "rand_low"
    };
