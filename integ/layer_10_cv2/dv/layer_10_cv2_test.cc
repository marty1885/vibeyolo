...
//
// layer_10_cv2_test — Integration test for tiled YOLO26n /model.4/cv2.
//
// Layer params (scale_pkg_dv):
//   COUT=128, CIN=96, K=1, P_COUT=64, P_CIN=8 → N_COUT_TILE=2, N_CIN_TILE=12
...
#include "Vlayer_10_cv2_tb.h"
#include "sim_ctrl.h"

using DUT = Vlayer_10_cv2_tb;

static constexpr int NCH_IN     = 96;
...
static constexpr int N_CIN_TILE  = NCH_IN  / P_CIN;         // 12
static constexpr int N_LANE_TILE = K * K * P_CIN;
static constexpr int N_LANE_FULL = K * K * NCH_IN;
...
static constexpr double S_OUT_SILU = 6.0 / 127.0;
...
int main(int argc, char** argv) {
    ...
    for (const auto& sname : sample_names) {
        ...
        {
            int wi = st_ort.worst_idx;
            int pix = wi / NCH_OUT, c = wi % NCH_OUT;
            printf("  worst idx=%d (pix=%d ch=%d) y_i8=%d dut_f=%.4f ref_ort=%.4f ref_hw=%.4f\n",
                   wi, pix, c, (int)y_i8[wi], st_ort.worst_dut, st_ort.worst_ref, s.ref_hw[wi]);
        }
        printf("  => %s\n", pass ? "PASS" : "FAIL");
