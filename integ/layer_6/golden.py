...
#
# golden.py — Stimulus + reference for tiled layer_6 (YOLO26n /model.3/conv
# Conv-BN-SiLU, 3x3, 64 -> 64, stride 2, pad 1). Input is /model.2/cv2 output.
...
NCH_IN, NCH_OUT, K, STRIDE = 64, 64, 3, 2
ROI_H, ROI_W = 16, 16
...
W_q  = init['onnx::Conv_1669_quantized'].astype(np.int32)
s_w  = float(init['onnx::Conv_1669_scale'])
zp_w = int(init['onnx::Conv_1669_zero_point'])
bias = init['onnx::Conv_1670'].astype(np.float32)
assert zp_w == 0 and W_q.shape == (NCH_OUT, NCH_IN, K, K)
sum_w = W_q.reshape(NCH_OUT, -1).sum(axis=1).astype(np.int32)

TARGET_OUT = '/model.3/act/Mul_output_0'
IN_Q  = '/model.2/cv2/act/Mul_output_0_quantized'
IN_S  = '/model.2/cv2/act/Mul_output_0_scale'
IN_ZP = '/model.2/cv2/act/Mul_output_0_zero_point'
    ...
    for c in range(NCH_OUT):
        for oy in range(OUT_H):
            for ox in range(OUT_W):
                ...
                w = x_i8[:, iy:iy+K, ix:ix+K].astype(np.int32)
                acc = int((W_q[c] * w).sum())
...
for sname, kind, seed in samples:
    ...
    if s_a == 0.0: s_a = 1.0
    ort_roi = ort_full[0, :, R//STRIDE:R//STRIDE+OUT_H, C//STRIDE:C//STRIDE+OUT_W]
