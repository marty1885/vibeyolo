...
//
// layer_7_cv1_test — Integration test for the tiled YOLO26n /model.4/cv1 layer.
//
// Layer params (scale_pkg_dv):
//   COUT=64, CIN=64, K=1, P_COUT=64, P_CIN=8
//   N_COUT_TILE=1, N_CIN_TILE=8
// Beats per output pixel = N_COUT_TILE * N_CIN_TILE = 8.
// For an 8x8 output ROI: 8*8*8 = 512 driver beats.
//
...
#include "Vlayer_7_cv1_tb.h"
#include "sim_ctrl.h"

using DUT = Vlayer_7_cv1_tb;

// Geometry — must match extract.py + layer_7_cv1.sv parameters
...
static constexpr int N_CIN_TILE  = NCH_IN  / P_CIN;         // 8
...
// PAD ring on the input plane (K=1 means no padding).
...
// Pipeline latency
static constexpr int DOT_LAT     = 1 + 3;  // 1 + clog2(8)
...
int main(int argc, char** argv) {
    ...
    const std::vector<std::string> sample_names = {
        "rand0", "rand1", "rand2", "rand3", "rand_low", "rand_high",
        "half", "gradient"
    };
