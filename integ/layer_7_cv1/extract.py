...
#
# extract.py — Generate stimulus + reference for the layer_7_cv1 integration
# test (YOLO26n /model.4/cv1: Conv 1x1, 64→64, stride 1, pad 0, followed by
# SiLU; entry of the second C3k2 block, operating on 80×80 feature maps).
#
# ONNX slice:
#   /model.3/act/Mul_output_0  (post-L6 SiLU, fp32)
#    → DynamicQuantizeLinear   (u8, s_a, zp_a)
#    → ConvInteger             (1x1, stride 1, pad 0, 64→64, w_zp=0)
#    → Cast i32→fp32
#    → Mul (s_a · s_w)
#    → Add bias
#    → Sigmoid → Mul (SiLU)
...
#
# Strategy mirrors integ/layer_2_cv1: take a small ROI on the 80×80 plane,
# drive the SV DUT with u8→i8 folded inputs, compare to ORT fp32 reference.
...
# ── Geometry ──
# Layer 7 cv1: 64→64, 1x1, stride 1, pad 0. Plane 80x80, drive an 8x8 ROI.
NCH_IN  = 64
NCH_OUT = 64
K       = 1
STRIDE  = 1
...
R, C = 16, 16   # top-left of crop in 80x80 layer-7-input space

# ── Load ONNX initializers ──
model = onnx.load(MODEL_PATH)
init_by_name = {init.name: numpy_helper.to_array(init) for init in model.graph.initializer}

W_q  = init_by_name['onnx::Conv_1672_quantized'].astype(np.int32)   # (64,64,1,1)
s_w  = float(init_by_name['onnx::Conv_1672_scale'])
zp_w = int(init_by_name['onnx::Conv_1672_zero_point'])
bias = init_by_name['onnx::Conv_1673'].astype(np.float32)           # (64,)
assert zp_w == 0
...
TARGET_OUT = '/model.4/cv1/act/Mul_output_0'              # layer-7-cv1 SiLU output
IN_Q       = '/model.3/act/Mul_output_0_quantized'        # u8 (1,64,80,80)
IN_S       = '/model.3/act/Mul_output_0_scale'            # scalar fp32
IN_ZP      = '/model.3/act/Mul_output_0_zero_point'       # uint8
...
# Pre-SiLU output of L7 cv1 typically lies in roughly the same range as the
# previous layer (post-SiLU). Pick a scale that gives ~1% quant step over the
# observed range. Will be refined automatically below.
S_OUT_PRE  = 4.0 / 127.0
...
# ── Weight packing ──
# 1x1: full reduction is NCH_IN=64. P_CIN=N_LANE=8 → N_PHASE=8.
N_LANE_FULL = NCH_IN          # 64
N_LANE      = 8               # P_CIN from scale_pkg LAYER_7_P_CIN
N_PHASE     = N_LANE_FULL // N_LANE  # 8
...
for sname, kind, seed in samples:
    ...
    if s_a == 0.0:
        s_a = 1.0
    ort_roi = ort_out_full[0, :, R:R+OUT_H, C:C+OUT_W]  # (64,8,8)
    hw_out, bias_eff, x_i8, s_acc = hw_reference(qx_u8_full, s_a, zp_a)

    # Per-channel fp16 scale and bias to feed requant.
    # layer_7_cv1.sv arithmetic-shift-rights sum_q by ACC_SHIFT (=2) before
    # requant to keep |acc| inside fp16 max (≈65504); empirical L7 worst-case
    # |sum_q| ≈ 68k (driven by the channel with largest |sum_w|). We
    # pre-multiply scale_fp16 by 2^ACC_SHIFT here so the arithmetic is
    # equivalent.
    # New tiled requant.sv autoscales internally; no ACC_SHIFT pre-mul needed.
    ACC_SHIFT = 0
    scale_fp16 = np.array(
