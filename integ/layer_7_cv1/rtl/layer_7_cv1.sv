...
//
// layer_7_cv1 — YOLO26n /model.4/cv1 Conv-BN-SiLU (1x1, 64->64, stride 1).
//
// Thin shim over the reusable conv_layer IP. All architecture lives in
// hw/ip/conv_layer; this file just binds scale_pkg::LAYER_7_* parameters.

// verilator lint_off UNUSEDPARAM
module layer_7_cv1
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_7_COUT,    // 64
  parameter int  CIN        = LAYER_7_CIN,     // 64
  parameter int  K          = LAYER_7_K,       // 1
  parameter int  STRIDE     = LAYER_7_STRIDE,  // 1
  parameter int  PAD        = LAYER_7_PAD,     // 0
  parameter int  P_COUT     = LAYER_7_P_COUT,  // 64
  parameter int  P_CIN      = LAYER_7_P_CIN,   // 8
  ...
  // Tie off residual ports — L7 has no residual.
  ...
  conv_layer #(
    ...
    .STRIDE    (STRIDE),
    .PAD       (PAD),
    .H_OUT     (LAYER_7_H / STRIDE),
    .W_OUT     (LAYER_7_W / STRIDE),
    .P_COUT    (P_COUT),
    .P_CIN     (P_CIN),
    .RESIDUAL  (0),
    .SILU      (1),
