...
//
// layer_16 — YOLO26n /model.6/m.0/m/m.0/cv2 Conv-BN-SiLU + residual Add
...
module layer_16
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_16_COUT,    // 32
  parameter int  CIN        = LAYER_16_CIN,     // 32
  parameter int  K          = LAYER_16_K,       // 3
  parameter int  STRIDE     = LAYER_16_STRIDE,
  parameter int  PAD        = LAYER_16_PAD,
  parameter int  P_COUT     = LAYER_16_P_COUT,
  parameter int  P_CIN      = LAYER_16_P_CIN,
  // Empirical: pre-SiLU ~[-2.6,2.06], |silu|~1.83, |add|~1.95
  parameter real S_OUT_PRE  = 3.119 / 127.0,
  parameter real S_OUT_SILU = 2.335 / 127.0
) (
  input  logic                                            clk_i,
  input  logic                                            rst_ni,
  ...
  conv_layer #(
    ...
    .H_OUT     (LAYER_16_H), .W_OUT (LAYER_16_W),
