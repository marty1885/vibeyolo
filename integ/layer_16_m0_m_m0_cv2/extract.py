...
#
# extract.py — Stimulus + reference for layer_16 (/model.6/m.0/m/m.0/cv2).
# 3x3, 32->32, stride 1, pad 1. Second conv of the first nested Bottleneck.
...
W_q  = inits['onnx::Conv_1696_quantized'].astype(np.int32)
s_w  = float(inits['onnx::Conv_1696_scale'])
zp_w = int(inits['onnx::Conv_1696_zero_point'])
bias = inits['onnx::Conv_1697'].astype(np.float32)
assert zp_w == 0
assert W_q.shape == (NCH_OUT, NCH_IN, K, K), f"got {W_q.shape}"
sum_w = W_q.reshape(NCH_OUT, -1).sum(axis=1).astype(np.int32)

TARGET_OUT = '/model.6/m.0/m/m.0/Add_output_0'
IN_Q  = '/model.6/m.0/m/m.0/cv1/act/Mul_output_0_quantized'
IN_S  = '/model.6/m.0/m/m.0/cv1/act/Mul_output_0_scale'
IN_ZP = '/model.6/m.0/m/m.0/cv1/act/Mul_output_0_zero_point'
RES_Q  = '/model.6/m.0/cv1/act/Mul_output_0_quantized'
RES_S  = '/model.6/m.0/cv1/act/Mul_output_0_scale'
RES_ZP = '/model.6/m.0/cv1/act/Mul_output_0_zero_point'
