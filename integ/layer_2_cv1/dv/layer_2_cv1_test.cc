...
//
// layer_2_cv1_test — Micro-integration test for YOLO26n /model.2/cv1.
// Loads ORT-derived stimulus + reference (../extract.py), runs layer_2_cv1.sv
// cycle-by-cycle (2 phases per output pixel), compares the int8 outputs
// (dequantised via S_OUT_SILU) to the ORT fp32 reference.
//   PASS: cosine > 0.998 AND mae < 5% of out_range.
...
#include "Vlayer_2_cv1_tb.h"
#include "sim_ctrl.h"

using DUT = Vlayer_2_cv1_tb;
...
static constexpr int N_LANE   = 16;            // P_CIN
static constexpr int N_PHASE  = NCH_IN / N_LANE;  // 2
...
static constexpr int TOTAL_LAT_FROM_PH1 =
...
struct Sample {
    std::string name;
    std::vector<int8_t>   input_i8;   // ROI_H*ROI_W*NCH_IN, order h,w,kc
...
// Build per-cycle half-window for (oy, ox, phase).
static void build_half_window(const Sample& s, int oy, int ox, int phase,
                              int8_t out[N_LANE]) {
...
int main(int argc, char** argv) {
    ...
    // Sample list. Random tiles are the primary pass criterion (cosine ≥ 0.998
    // vs ORT, per spec). Low-dynamic-range corner tiles (half, gradient) are
    // included for informational tracking; they are dominated by the SiLU LUT
    // step and held to a looser cosine bound — same behaviour observed in
    // integ/layer_1.
    ...
    for (const auto& se : sample_names) {
        ...
        std::vector<int8_t> w_ph0(NCH_OUT * N_LANE), w_ph1(NCH_OUT * N_LANE);
        build_w_phase(w_bytes, 0, w_ph0.data());
        build_w_phase(w_bytes, 1, w_ph1.data());
        ...
        int produced = 0;
        // outputs come out (TOTAL_LAT_FROM_PH1) cycles after each phase-1
        // input.  We track expected pixel index per phase-1 tick.
        std::vector<int> inflight;
        inflight.reserve(N_OUT + TOTAL_LAT_FROM_PH1);

        int cycles_used = 0;

        for (int step = 0; step < N_PHASES_TOT + TOTAL_LAT_FROM_PH1 + 8; step++) {
        ...
        // Required (random) tiles: cosine ≥ 0.998 vs ORT. Informational
        // (low-dynamic-range) tiles: cosine ≥ 0.99 only (SiLU LUT-dominated).
    ...
    const int FRAME_H = 160, FRAME_W = 160;
    const int FRAME_PIX = FRAME_H * FRAME_W;
    const int CYCLES_PER_PIX = N_PHASE;                  // 2
    const int FRAME_CYCLES = FRAME_PIX * CYCLES_PER_PIX; // 51200
    const int MACS_PER_FRAME = FRAME_PIX * NCH_OUT * NCH_IN; // 25600*32*32 = 26_214_400
    const int MAC_UNITS = NCH_OUT * N_LANE;              // 32 * 16 = 512
    ...
    printf("  worst max   = %.6f\n", agg_max);
    printf("Cycle accounting (layer_2_cv1, 1x1 conv 32→32 @ 160x160):\n");
    ...
    printf("  total MACs           : %d\n", MACS_PER_FRAME);
    printf("  frame cycles         : %d  (matches LAYER_2_CYCLES = 51200, target T_FRAME=100000)\n",
           FRAME_CYCLES);
    printf("========================================\n");
