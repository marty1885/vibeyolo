...
#
# golden.py — Generate stimulus + reference for the layer_3_m0_cv1 integration
# test. Slice: YOLO26n /model.2/m.0/cv1 Conv-BN-SiLU (the first conv inside
# the C3k2 bottleneck of /model.2).
#
#   /model.2/Slice_1_output_0_quantized  (u8, 16ch, 160x160, s_a, zp_a)
#    → ConvInteger (16→8, 3x3, stride 1, pad 1, zp_w=0)
#    → Cast i32→fp32 → Mul (s_a·s_w) → Add bias
#    → Sigmoid → Mul   (SiLU)
#
# The conv input is the second half of /model.2/cv1's 32-channel output
# (C3k2's split). We treat it as a given 16-channel u8 tensor and pull both
...
# Geometry: L3 = 16→8, 3x3, stride 1, pad 1. Input 160x160 → output 160x160.
# Drive an 8x8 output ROI inside the 160x160 grid (input ROI is 10x10×16 with
...
PAD_H, PAD_W = ROI_H + 2, ROI_W + 2     # 10x10, includes 1-pixel pad ring
# Position inside the 160x160 layer-3-input space. With pad=1, output (R,C)
# uses input window centered at (R,C), so we pull rows R-1..R+8, cols C-1..C+8.
R, C = 40, 40

# ── Load ONNX initializers (layer-3 conv params) ──
model = onnx.load(MODEL_PATH)
init_by_name = {init.name: numpy_helper.to_array(init) for init in model.graph.initializer}

W_q  = init_by_name['onnx::Conv_1660_quantized'].astype(np.int32)   # (8,16,3,3)
s_w  = float(init_by_name['onnx::Conv_1660_scale'])
zp_w = int(init_by_name['onnx::Conv_1660_zero_point'])
bias = init_by_name['onnx::Conv_1661'].astype(np.float32)            # (8,)
assert zp_w == 0
assert W_q.shape == (NCH_OUT, NCH_IN, K, K), f"Got {W_q.shape}"

sum_w = W_q.reshape(NCH_OUT, -1).sum(axis=1).astype(np.int32)        # (8,)

# ── ORT session with extra outputs ──
TARGET_OUT = '/model.2/m.0/cv1/act/Mul_output_0'              # layer-3 post-SiLU
L3_IN_Q    = '/model.2/Slice_1_output_0_quantized'            # u8 (1,16,160,160)
L3_IN_S    = '/model.2/Slice_1_output_0_scale'                # scalar fp32
L3_IN_ZP   = '/model.2/Slice_1_output_0_zero_point'           # uint8

EXTRA_OUTS = [
    (TARGET_OUT, onnx.TensorProto.FLOAT, None),
    (L3_IN_Q,    onnx.TensorProto.UINT8, None),
    (L3_IN_S,    onnx.TensorProto.FLOAT, None),
    (L3_IN_ZP,   onnx.TensorProto.UINT8, None),
]
...
def run_ort(img_f32):
    outs = sess.run([TARGET_OUT, L3_IN_Q, L3_IN_S, L3_IN_ZP], {ipt_name: img_f32})
...
# ── Per-channel output scales (pre- and post-SiLU). Activations of an internal
# C3k2 inner-conv are typically smaller than the stem. We size for ±32 which
# covers the observed range comfortably (LUT step ≈ 32/127 ≈ 0.252).
S_OUT_PRE  = 2.0 / 127.0
S_OUT_SILU = 2.0 / 127.0

def f32_to_fp16(x):  return np.float16(x).view(np.uint16)

def hw_reference(qx_in_u8_full, s_a, zp_a):
    """qx_in_u8_full: (1,16,160,160) uint8 — ORT's L3-input quantization."""
    s_acc = s_a * s_w
    ...
    bias_eff = bias + s_acc * (128 - zp_a) * sum_w       # (8,)
...
for sname, kind, seed in samples:
    ...
    if s_a == 0.0:
        s_a = 1.0
    ort_roi = ort_out_full[0, :, R:R+OUT_H, C:C+OUT_W]      # (8,8,8)
