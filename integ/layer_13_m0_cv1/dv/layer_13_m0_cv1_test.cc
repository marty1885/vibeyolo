...
//
// layer_13_m0_cv1_test — Micro-integration test for YOLO26n
// /model.6/m.0/cv1. Loads ORT-derived stimulus + reference (../extract.py),
// runs layer_13_m0_cv1.sv cycle-by-cycle (32 phases per output pixel),
// compares the int8 outputs (dequantised via S_OUT_SILU) to the ORT fp32
// reference.
...
#include "Vlayer_13_m0_cv1_tb.h"
#include "sim_ctrl.h"

using DUT = Vlayer_13_m0_cv1_tb;
...
static constexpr int N_LANE   = 2;                 // P_CIN
...
static constexpr int DOT_LAT     = 2;   // 1 + clog2(2)
...
int main(int argc, char** argv) {
    ...
    const int MACS_PER_FRAME = FRAME_PIX * NCH_OUT * NCH_IN; // 1600*32*64 = 3_276_800
    const int MAC_UNITS = NCH_OUT * N_LANE;              // 32 * 2 = 64
    ...
    printf("  worst max   = %.6f\n", agg_max);
    printf("Cycle accounting (layer_13_m0_cv1, 1x1 conv 64→32 @ 40x40):\n");
    ...
    printf("  total MACs           : %d\n", MACS_PER_FRAME);
    printf("  frame cycles         : %d  (matches LAYER_13_CYCLES = 51200, target T_FRAME=100000)\n",
           FRAME_CYCLES);
    printf("========================================\n");
