...
#
# Attention block extractor for YOLO26n /model.10 (PSA).
#
# This block sits BETWEEN the five already-validated convs L34..L38. The
# DUT consumes the convs' float outputs (quantized to int8 at this
# block's input boundaries) and reproduces the inter-conv graph:
#
#  Inputs (with per-tensor int8 scales):
#    QKV    : output of L34 (qkv conv)        — 256ch × 20 × 20
#    PE     : output of L35 (pe depthwise)    — 128ch × 20 × 20
#    PROJ   : output of L36 (proj conv)       — 128ch × 20 × 20
#    SPLIT1 : /model.10/Split_output_1        — 128ch × 20 × 20  (residual src)
#    FFN1   : output of L38 (ffn.1 conv)      — 128ch × 20 × 20  (residual src)
#
#  Topology (verified from ONNX):
#    Reshape QKV [1,256,20,20] → [1,2,128,400]            (B, H=2, C=128, N=400)
#    Split   along dim 2 with sizes [32,32,64]            (Q[32],K[32],V[64])
#    Transpose(Q)  perm=[0,1,3,2]                          → [1,2,400,32]
#    Reshape (V)  → [1,128,20,20] (PE input)
#    MatMul scores  Q^T @ K                                → [1,2,400,400]
#    Mul    × 1/sqrt(32) = 0.17677669
#    Softmax along last axis (400-way per row)
#    Transpose(scores) perm=[0,1,3,2]                      → [1,2,400,400]
#    MatMul  V @ scores^T                                  → [1,2,64,400]
#    Reshape → [1,128,20,20]
#    Add  + PE                                             → ATTN_OUT [1,128,20,20]
#    (proj conv L36 runs externally)
#    Add  + SPLIT1                                         → PROJ_RES [1,128,20,20]
#    (ffn convs L37/L38 run externally)
#    Add  + FFN1 output                                    → ATTN_FINAL [1,128,20,20]
#
...
#
# Comparison: cos(DUT_dequant vs ORT_float) on the full 20x20 frame.
...
C_FE  = HEADS * DIM_V                      # 128 (V-side, == post-attn channel count)
SOFTMAX_SCALE = 1.0 / math.sqrt(DIM_Q)     # ≈ 0.17677669

# Tensor names from the ONNX graph.
QKV_OUT    = "/model.10/m/m.0/attn/qkv/conv/Conv_output_0"     # B×256×20×20 (post-bias)
PE_OUT     = "/model.10/m/m.0/attn/pe/conv/Conv_output_0"      # B×128×20×20 (post-bias)
PROJ_OUT   = "/model.10/m/m.0/attn/proj/conv/Conv_output_0"    # B×128×20×20 (post-bias)
SPLIT1_OUT = "/model.10/Split_output_1"                         # B×128×20×20
FFN1_OUT   = "/model.10/m/m.0/ffn/ffn.1/conv/Conv_output_0"    # B×128×20×20 (post-bias)
# Block boundaries we will compare against:
ATTN_ADD   = "/model.10/m/m.0/attn/Add_output_0"               # post pe-add (input to proj)
PROJ_RES   = "/model.10/m/m.0/Add_output_0"                    # post proj-residual (input to ffn.0)
FINAL_ADD  = "/model.10/m/m.0/Add_1_output_0"                  # final block output
...
# ── Software model of the attention math (fp16, matching the DUT) ──
def sw_attn(qkv_f32, pe_f32):
    """qkv_f32: (256, 20, 20)  pe_f32: (128, 20, 20)
    Returns float32 ATTN_ADD output (128, 20, 20)."""
    # Reshape qkv → (heads=2, c=128, N=400)
    qkv = qkv_f32.reshape(HEADS, DIM_Q + DIM_K + DIM_V, N)
    Q = qkv[:, 0:DIM_Q, :]                              # (2, 32, 400)
    K = qkv[:, DIM_Q:DIM_Q + DIM_K, :]                  # (2, 32, 400)
    V = qkv[:, DIM_Q + DIM_K:, :]                       # (2, 64, 400)

    # Transpose Q: (2, 400, 32). MatMul scores = Q^T @ K → (2, 400, 400).
    scores = np.einsum("hdn,hdm->hnm", Q, K).astype(np.float32)
    scores *= np.float32(SOFTMAX_SCALE)
    # Softmax along last axis. Numerical stability with row-wise max.
    m = scores.max(axis=-1, keepdims=True)
    e = np.exp((scores - m).astype(np.float64))
    p = (e / e.sum(axis=-1, keepdims=True)).astype(np.float32)  # (2, 400, 400)
    # Transpose softmax along last two axes → (2, 400, 400).
    pT = np.transpose(p, (0, 2, 1))
    # MatMul: V @ pT → (2, 64, 400).
    out = np.einsum("hdn,hnm->hdm", V, pT).astype(np.float32)
    out = out.reshape(HEADS * DIM_V, H, W)              # (128, 20, 20)
    return out + pe_f32
...
# Per-tensor scales: max*1.1/127 rule.
def sc(nm):
    return max(max_abs[nm] * 1.1 / 127.0, 1e-3 / 127.0)


# Per-sample S_OUT (final tensor). We use the FINAL_ADD scale.
S_QKV   = sc(QKV_OUT)
S_PE    = sc(PE_OUT)
S_PROJ  = sc(PROJ_OUT)
S_SPL1  = sc(SPLIT1_OUT)
S_FFN1  = sc(FFN1_OUT)
S_AOUT  = sc(ATTN_ADD)        # this is what proj will read as input
S_RES   = sc(PROJ_RES)
S_OUT   = sc(FINAL_ADD)
...
def sw_block_end_to_end(qkv_i8, pe_i8, proj_i8, spl1_i8, ffn1_i8):
    """Software fp16-ish reference of the FULL block (mirrors what the
    DUT does). Dequantize int8 inputs, run fp16 attention math,
    requantize each boundary, run downstream residuals. Returns the
    final int8 frame and the two intermediate int8 frames so the DUT
    can be compared at every boundary if needed."""
    ...
    attn_out_f = sw_attn(qkv, pe)                # (128,20,20) float
    attn_out_i8 = quantize_i8(attn_out_f, S_AOUT)

    res_f = (attn_out_i8.astype(np.float32) * S_AOUT) * 0.0 + (
        proj + spl1
    )  # in float; real chip's add_rq does this in fp16 too.
    # Use proj + spl1 (the conv float outputs) for fidelity vs ORT:
    res_f = proj + spl1
...
for (sname, res) in ort_cache:
    ...
    # Also dump the attention-only boundary (for diagnostics if needed).
...
# Emit a SystemVerilog package that pins all per-tensor scales to the
# DUT at elaboration time. This is read by attn_tb.sv (which feeds the
# parameters into attn.sv).
def fp16_bits(x):
    return int(np.float16(x).view(np.uint16))

pkg_path = os.path.join(STIM, "attn_scales_pkg.sv")
with open(pkg_path, "w") as f:
    f.write("// AUTO-GENERATED by integ/attn_model10/extract.py — do not edit.\n")
    f.write("package attn_scales_pkg;\n")
    # Forward dequant scales (int8 * s -> fp16 float-domain value), as fp16 bits.
    ...
    # Inverse output scales (fp16 -> int8 via fp16_fma(x, inv, 0) then sat_i8).
    ...
    # Real-typed copies for any tooling that still wants them (kept for back-compat).
