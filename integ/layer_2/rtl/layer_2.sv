...
//
// layer_2 — YOLO26n /model.2/cv1 Conv-BN-SiLU (1x1, stride 1, 32->32).
...
module layer_2
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_2_COUT,
  parameter int  CIN        = LAYER_2_CIN,
  parameter int  K          = LAYER_2_K,
  parameter int  STRIDE     = LAYER_2_STRIDE,
  parameter int  PAD        = LAYER_2_PAD,
  parameter int  P_COUT     = 16,
  parameter int  P_CIN      = 8,
  parameter real S_OUT_PRE  = 24.0 / 127.0,
  parameter real S_OUT_SILU = 24.0 / 127.0
) (
  input  logic                                            clk_i,
  input  logic                                            rst_ni,
