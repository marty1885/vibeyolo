...
//
// layer_19 — YOLO26n /model.6/m.0/cv3 Conv-BN-SiLU (1x1, stride 1, 64→64).
// Output conv of the C3k (c3k=True) bottleneck wrapper /model.6/m.0.
// No residual; output feeds the outer /model.6/Concat.
// Thin shim over conv_layer with scale_pkg::LAYER_19_*.

// verilator lint_off UNUSEDPARAM
module layer_19
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_19_COUT,    // 64
  parameter int  CIN        = LAYER_19_CIN,     // 64
  parameter int  K          = LAYER_19_K,       // 1
  parameter int  STRIDE     = LAYER_19_STRIDE,  // 1
  parameter int  PAD        = LAYER_19_PAD,     // 0
  parameter int  P_COUT     = LAYER_19_P_COUT,  // 64
  parameter int  P_CIN      = LAYER_19_P_CIN,   // 2
  // S_OUT_PRE/SILU picked from extract.py's empirical pre/post-SiLU range.
  parameter real S_OUT_PRE  = 5.275 / 127.0,
  parameter real S_OUT_SILU = 3.584 / 127.0
) (
  input  logic                                            clk_i,
  input  logic                                            rst_ni,
  ...
  // Tie off residual ports — L14 has no residual.
