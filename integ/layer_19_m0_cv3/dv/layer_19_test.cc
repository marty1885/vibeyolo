...
//
// layer_19_test — Integration test for the YOLO26n /model.6/m.0/cv2 layer
// (1×1, 64→64, non-residual). Per-tile streaming, pattern from layer_11.
...
#include "Vlayer_19_tb.h"
#include "sim_ctrl.h"

using DUT = Vlayer_19_tb;
...
static constexpr int N_COUT_TILE = NCH_OUT / P_COUT;       // 1
static constexpr int N_CIN_TILE  = NCH_IN  / P_CIN;        // 32
static constexpr int N_LANE_TILE = K * K * P_CIN;          // 2
static constexpr int N_LANE_FULL = K * K * NCH_IN;         // 64
...
static constexpr double S_OUT_SILU = 3.584 / 127.0;
