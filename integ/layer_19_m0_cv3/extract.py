...
#
# extract.py — Generate stimulus + reference for the layer_19_m0_cv3 integ test.
#
# Slice: YOLO26n /model.6/m.0/cv3 Conv-BN-SiLU — output conv of the C3k
# wrapper at /model.6/m.0. K=1, 64→64, stride 1, pad 0, 40×40 plane. Same
# input plane as L13 (cv1 sibling).
#
# Pipeline through the model:
#   /model.6/m.0/Concat_output_0_quantized (u8, s_a, zp_a, 1×64×40×40)
...
W_q  = inits['onnx::Conv_1708_quantized'].astype(np.int32)
s_w  = float(inits['onnx::Conv_1708_scale'])
zp_w = int(inits['onnx::Conv_1708_zero_point'])
bias = inits['onnx::Conv_1709'].astype(np.float32)
assert zp_w == 0
assert W_q.shape == (NCH_OUT, NCH_IN, K, K), f"got {W_q.shape}"

sum_w = W_q.reshape(NCH_OUT, -1).sum(axis=1).astype(np.int32)

TARGET_OUT = '/model.6/m.0/cv3/act/Mul_output_0'
IN_Q  = '/model.6/m.0/Concat_output_0_quantized'
IN_S  = '/model.6/m.0/Concat_output_0_scale'
IN_ZP = '/model.6/m.0/Concat_output_0_zero_point'
