...
//
// layer_20_test — Integration test for the YOLO26n /model.6/cv2 layer
// (1×1, 192→128, non-residual). Per-tile streaming, pattern from layer_11.
...
#include "Vlayer_20_tb.h"
#include "sim_ctrl.h"

using DUT = Vlayer_20_tb;

static constexpr int NCH_IN     = 192;
static constexpr int NCH_OUT    = 128;
static constexpr int K          = 1;
static constexpr int STRIDE     = 1;
static constexpr int P_COUT     = 64;
static constexpr int P_CIN      = 4;
static constexpr int N_COUT_TILE = NCH_OUT / P_COUT;       // 2
static constexpr int N_CIN_TILE  = NCH_IN  / P_CIN;        // 48
static constexpr int N_LANE_TILE = K * K * P_CIN;          // 4
static constexpr int N_LANE_FULL = K * K * NCH_IN;         // 192
...
static constexpr double S_OUT_SILU = 2.062 / 127.0;

static constexpr int DOT_LAT     = 1 + 1;   // 1 + clog2(2) = 2
...
// K=1: lane = kc_local (no spatial component). Window is a single pixel.
static void build_window_tile(const Sample& s, int oy, int ox, int cit,
    ...
    for (int kc_local = 0; kc_local < P_CIN; kc_local++) {
        ...
        out[kc_local] = s.input_i8[in_off];
    }
}
// w_full layout (NCH_OUT, N_LANE_FULL=NCH_IN), lane=kc.
static void build_weight_tile(const std::vector<int8_t>& w_full, int ct, int cit,
    ...
    for (int co_local = 0; co_local < P_COUT; co_local++) {
        ...
        for (int kc_local = 0; kc_local < P_CIN; kc_local++) {
            int kc = kc_base + kc_local;
            out[co_local * N_LANE_TILE + kc_local] = w_full[co_global * N_LANE_FULL + kc];
        }
    }
}
