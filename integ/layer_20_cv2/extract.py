...
#
# extract.py — Generate stimulus + reference for the layer_20_cv2 integ test.
#
# Slice: YOLO26n /model.6/cv2 Conv-BN-SiLU — exit conv of the C3k2 block.
# K=1, 192→128, stride 1, pad 0, 40×40 plane.
#
# Pipeline through the model:
#   /model.6/Concat_output_0_quantized (u8, s_a, zp_a, 1×192×40×40)
#    → ConvInteger (128×192×1×1, zp_w = 0)
#    → ×(s_a * s_w) → +bias → SiLU
#    → /model.6/cv2/act/Mul_output_0
#
# Non-residual layer; block output feeds /model.7/conv.
...
W_q  = inits['onnx::Conv_1711_quantized'].astype(np.int32)
s_w  = float(inits['onnx::Conv_1711_scale'])
zp_w = int(inits['onnx::Conv_1711_zero_point'])
bias = inits['onnx::Conv_1712'].astype(np.float32)
assert zp_w == 0
assert W_q.shape == (NCH_OUT, NCH_IN, K, K), f"got {W_q.shape}"

sum_w = W_q.reshape(NCH_OUT, -1).sum(axis=1).astype(np.int32)

TARGET_OUT = '/model.6/cv2/act/Mul_output_0'
IN_Q  = '/model.6/Concat_output_0_quantized'
IN_S  = '/model.6/Concat_output_0_scale'
IN_ZP = '/model.6/Concat_output_0_zero_point'
