...
#
# extract.py — Stimulus + reference for layer_17 (/model.6/m.0/m/m.1/cv1).
# First conv of second nested bottleneck (3x3, 32->32, stride 1, pad 1) inside
# the C3k=True bottleneck wrapper /model.6/m.0. Input plane is the first nested bottleneck Add
# output: /model.6/m.0/m/m.0/Add_output_0 (u8, 32ch, 40x40).
...
W_q  = inits['onnx::Conv_1699_quantized'].astype(np.int32)
s_w  = float(inits['onnx::Conv_1699_scale'])
zp_w = int(inits['onnx::Conv_1699_zero_point'])
bias = inits['onnx::Conv_1700'].astype(np.float32)
assert zp_w == 0
assert W_q.shape == (NCH_OUT, NCH_IN, K, K), f"got {W_q.shape}"

sum_w = W_q.reshape(NCH_OUT, -1).sum(axis=1).astype(np.int32)

TARGET_OUT = '/model.6/m.0/m/m.1/cv1/act/Mul_output_0'
IN_Q  = '/model.6/m.0/m/m.0/Add_output_0_quantized'
IN_S  = '/model.6/m.0/m/m.0/Add_output_0_scale'
IN_ZP = '/model.6/m.0/m/m.0/Add_output_0_zero_point'
