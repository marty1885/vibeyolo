...
//
// layer_17 — YOLO26n /model.6/m.0/m/m.0/cv1 Conv-BN-SiLU (3x3, s=1, 32→32).
...
module layer_17
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_17_COUT,    // 32
  parameter int  CIN        = LAYER_17_CIN,     // 32
  parameter int  K          = LAYER_17_K,       // 3
  parameter int  STRIDE     = LAYER_17_STRIDE,  // 1
  parameter int  PAD        = LAYER_17_PAD,     // 1
  parameter int  P_COUT     = LAYER_17_P_COUT,  // 32
  parameter int  P_CIN      = LAYER_17_P_CIN,   // 1
  // Empirical: pre-SiLU ~[-4.85, 2.02], post-SiLU |max| ~1.78
  parameter real S_OUT_PRE  = 3.942 / 127.0,
  parameter real S_OUT_SILU = 2.000 / 127.0
) (
  input  logic                                            clk_i,
  input  logic                                            rst_ni,
  ...
  conv_layer #(
    ...
    .H_OUT     (LAYER_17_H), .W_OUT (LAYER_17_W),
