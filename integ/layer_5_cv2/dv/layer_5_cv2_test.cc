...
//
// layer_5_cv2_test — Micro-integration test for YOLO26n /model.2/cv2
// Conv 1×1 (48→64) + SiLU. Loads ORT-derived stimulus + reference (via
// ../extract.py), runs layer_5_cv2.sv cycle-by-cycle, compares int8 outputs
...
#include "Vlayer_5_cv2_tb.h"
#include "sim_ctrl.h"

using DUT = Vlayer_5_cv2_tb;
...
static constexpr int N_LANE   = K * K * NCH_IN;   // 48
static constexpr int ROI_H    = 8;
static constexpr int ROI_W    = 8;
static constexpr int PAD_H    = ROI_H;            // K=1: no pad ring
...
// Build per-cycle 48-lane window: lane = kc (K=1).
...
int main(int argc, char** argv) {
    ...
    printf("  worst max   = %.6f\n", agg_max);
    printf("Cycle accounting (layer_5_cv2 unit test, %dx%d ROI):\n", OUT_H, OUT_W);
    ...
    printf("  ROI (%dx%d = %d pix)  total cycles : %d\n",
           OUT_H, OUT_W, OUT_H * OUT_W, OUT_H * OUT_W + TOTAL_LAT - 1);
    printf("Frame cycles at full scale (160x160 out = 25600 pix, "
           "P_PIX=1, P_COUT=64, P_CIN=16):\n");
    printf("  expected ~%d cycles (matches scale_pkg::LAYER_5_CYCLES)\n",
           (NCH_IN * K * K / 16) * 160 * 160);
    printf("========================================\n");
