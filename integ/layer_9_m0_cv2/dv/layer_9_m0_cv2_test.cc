...
//
// layer_9_m0_cv2_test — Integration test for the tiled YOLO26n /model.4/m.0/cv2
// + bottleneck residual add.
//
// Layer params (scale_pkg_dv):
//   COUT=32, CIN=16, K=3, P_COUT=32, P_CIN=2 → N_COUT_TILE=1, N_CIN_TILE=8
...
#include "Vlayer_9_m0_cv2_tb.h"
#include "sim_ctrl.h"

using DUT = Vlayer_9_m0_cv2_tb;
...
static constexpr int N_COUT_TILE = NCH_OUT / P_COUT;
static constexpr int N_CIN_TILE  = NCH_IN  / P_CIN;
static constexpr int N_LANE_TILE = K * K * P_CIN;          // 18
...
static constexpr double S_OUT_ADD = 6.5 / 127.0;  // final output scale (post add_rq)

static constexpr int DOT_LAT     = 1 + 5;  // 1 + clog2(18)
...
static constexpr int ADDRQ_LAT   = 6;
...
struct Sample {
    ...
    uint16_t              sa_fp16;
    uint16_t              sb_fp16;
    uint16_t              inv_out_fp16;
    uint16_t              bias_add_fp16;
...
static Sample load_sample(const std::string& name) {
    ...
    s.sa_fp16       = (uint16_t)sc[0];
    s.sb_fp16       = (uint16_t)sc[1];
    s.inv_out_fp16  = (uint16_t)sc[2];
    s.bias_add_fp16 = (uint16_t)sc[3];
...
int main(int argc, char** argv) {
    ...
    pack_u16(sim.dut->rs_flat_i, zero_u16.data(), P_COUT);
    pack_u16(sim.dut->rb_flat_i, zero_u16.data(), P_COUT);
    sim.reset();
    ...
    for (const auto& sname : sample_names) {
        ...
        for (int step = 0; step < TOTAL_BEATS + TOTAL_LAT + 16; step++) {
            if (step < TOTAL_BEATS) {
                ...
                // Residual: held steady across cin sweep for this (pix, ct).
                int8_t   rbuf[P_COUT];
                uint16_t rs_buf[P_COUT], rb_buf[P_COUT];
                for (int c = 0; c < P_COUT; c++) {
                    int co = co_base + c;
                    rbuf[c]   = s.resid_i8[(oy * OUT_W + ox) * NCH_OUT + co];
                    rs_buf[c] = s.sb_fp16;
                    rb_buf[c] = s.bias_add_fp16;
                }
                pack_bytes(sim.dut->r_flat_i, rbuf, P_COUT);
                pack_u16(sim.dut->rs_flat_i, rs_buf, P_COUT);
                pack_u16(sim.dut->rb_flat_i, rb_buf, P_COUT);
