...
//
// layer_11 — YOLO26n /model.5/conv Conv-BN-SiLU (3x3, stride 2, 128->128).
//
// Thin shim over the reusable conv_layer IP. All architecture lives in
// hw/ip/conv_layer; this file just binds scale_pkg::LAYER_11_* parameters.

// verilator lint_off UNUSEDPARAM
module layer_11
  import scale_pkg::*;
#(
  parameter int  COUT       = LAYER_11_COUT,    // 128
  parameter int  CIN        = LAYER_11_CIN,     // 128
  parameter int  K          = LAYER_11_K,       // 3
  parameter int  STRIDE     = LAYER_11_STRIDE,  // 2
  parameter int  PAD        = LAYER_11_PAD,     // 1
  parameter int  P_COUT     = 16,               // layer_11 dv uses 16/8 (not pkg's 64/4)
  ...
  // Tie off residual ports — L11 has no residual.
  ...
  conv_layer #(
    ...
    .STRIDE    (STRIDE),
    .PAD       (PAD),
    .H_OUT     (LAYER_11_H / STRIDE),
    .W_OUT     (LAYER_11_W / STRIDE),
    .P_COUT    (P_COUT),
    .P_CIN     (P_CIN),
    .RESIDUAL  (0),
    .SILU      (1),
