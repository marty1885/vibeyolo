...
//
// act_sigmoid — int8 → int8 sigmoid activation via a 256-entry ROM lookup.
//
// sigmoid(x) = 1 / (1 + exp(-x)).
//
// The DUT realises the activation by baking the float sigmoid at
// elaboration into a 256-entry signed-int8 ROM. The input x_i is
// interpreted as a signed int8 with scale `InScale` (the float-domain
// value is signed(x_i) * InScale); the output y_o is a signed int8 with
// scale `OutScale` (so the float-domain sigmoid value is
// signed(y_o) * OutScale). The output is mathematically in (0, 1), so a
// natural OutScale is 1/128: sigmoid range [0, 1] → quantised [0, 127].
//
// For each codepoint i ∈ [-128, 127]:
//   f = real'(i) * InScale
//   s = 1 / (1 + exp(-f))                    // float sigmoid
...
module act_sigmoid #(
  ...
  function automatic logic signed [7:0] rne_sat_i8(input real v);
    ...
    // Round-half-to-even. Sigmoid produces irrational values in general,
    // so the half-tie branch is defensively present but never taken at
  ...
  initial begin : g_lut_init
    ...
    for (idx = 0; idx < 256; idx = idx + 1) begin
      ...
      // sigmoid(f) = 1/(1+exp(-f)). Compute robustly for large |f|:
      // for f < 0, rewrite as exp(f)/(1+exp(f)) to avoid huge exp(-f).
      if (f >= 0.0) begin
        s = 1.0 / (1.0 + $exp(-f));
      end else begin
        ef = $exp(f);
