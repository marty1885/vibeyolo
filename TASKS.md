...
- [~] **`hw/ip/flash_attn/` — flash-attention leaf IP (WIP, over budget)** — parameterized (HEADS, N, DIM_Q, DIM_V, TEMP) tile-streaming fp16 flash-attention with online softmax. Algorithm + golden + DV scaffolding all correct: 5/5 checks pass at HEADS=1 / N=8 / DIM_Q=4 / DIM_V=4, 18 frames, 608 output words, 0 dut-vs-ref fails at tol 64 ULP / abs 0.01, worst cos(dut, shadow)=0.999976. **Throughput not met**: current BR=1 BC=1 (~162 fp16_fma cells) → ~2 M cyc/frame at YOLO26n shapes — **20× over T_FRAME=100k**. Boundary, golden, and DV are parametric — unroll is straight-line follow-up below. **Resolved**: pe (depthwise on V) stays outside the IP — flash_attn takes Q/K/V → O.
- [ ] **`hw/ip/flash_attn/` parallelization push** *(next up — top priority)* — unroll BR/BC to hit T_FRAME=100k at YOLO26n shapes (HEADS=2, N=400, DIM_Q=32, DIM_V=64). Target P_FMA ~512 (e.g. BR=32 BC=16) → ~60k cyc/pass. Re-validate DV at small shapes (existing tests must still pass) AND at production shapes (add a new test variant). Boundary/golden stay as-is.
- [ ] **Attention integration shim — `/model.10` PSA** *(after flash_attn hits budget)* — thin shim around `flash_attn` (~80 lines RTL): QKV split off L34's output, drive flash_attn, pe-add post-attention, requant boundaries, residuals via `add_rq` around external proj/FFN convs. Replaces the behavioral `integ/attn_model10/`; re-runs the existing extract.py / TB against ORT.
