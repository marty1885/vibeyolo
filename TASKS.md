...
- [ ] **Detect head with learned top-k** — end-to-end, no NMS. Largest remaining technical risk. Groundwork: `hw/ip/topk_fp16` leaf IP done (small min-heap, 75,981 cyc/frame for N=8400 K=300 sorted-ascending worst case). Remaining: sigmoid → reduce_max → topk → gather shim around `/model.23`.
